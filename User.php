...
public function createUser($matric, $name, $password, $role)
{
    ...
    $sqlCheck = "SELECT * FROM users WHERE matric = ?";
    $stmtCheck = $this->conn->prepare($sqlCheck);

    if ($stmtCheck) {
        $stmtCheck->bind_param("s", $matric);
        $stmtCheck->execute();
        $resultCheck = $stmtCheck->get_result();

        if ($resultCheck->num_rows > 0) {
            $stmtCheck->close();
            return "This matric has already been registered.";
        }
        $stmtCheck->close();
    } else {
    ...
    // Proceed with registration if matric does not exist
